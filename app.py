...
# =========================
# A안) 상단 2개 탭(관리자/학생)으로 화면 분리
# - 관리자는 관리자/학생 화면 둘 다 볼 수 있음
# - 학생은 학생 화면만 보임
# =========================
...
def student_tab_visible(tab_name: str, perms: dict) -> bool:
    # 학생은 기본 "내 통장" + 일정(읽기)
    if tab_name == "🏦 내 통장":
        return True
    if tab_name == "🗓️ 일정":
    ...
    if tab_name == "🏛️ 국세청(국고)":
        return can(perms, "treasury_read") or can(perms, "treasury_write")
    if tab_name == "📊 통계청":
        return can(perms, "stats_write")
    if tab_name == "💳 신용등급":
        return can(perms, "credit_write")
    if tab_name == "🏦 은행(적금)":
        return can(perms, "bank_read") or can(perms, "bank_write")

    # 학생에게는 기본 허용(원하면 여기서 더 제한 가능)
    if tab_name in ("📈 투자", "🛒 구입/벌금"):
        return True
...
def build_tabs(view_is_admin: bool):
    if view_is_admin:
        tabs = ADMIN_TABS
    ...
    # ✅ 학생 화면(권한 필터 적용)
    perms = get_my_permissions(my_student_id, is_admin=False)
    tabs = [t for t in STUDENT_TABS if student_tab_visible(t, perms)]
    tab_objs = st.tabs(tabs)
    tab_map = {name: tab_objs[i] for i, name in enumerate(tabs)}
    return tabs, tab_map


def render_tab_contents(tabs, tab_map, view_is_admin: bool):
    # ✅ A안: 관리자/학생 화면 분리 탭 (학생 화면에서는 권한을 '학생' 기준으로 동작)
    is_admin = view_is_admin
    my_perms = get_my_permissions(my_student_id, is_admin=is_admin)
...
# --- 상단 모드 탭 ---
if is_admin:
    mode_admin, mode_student = st.tabs(["👩‍🏫 관리자", "🧒 학생"])

    with mode_admin:
        tabs, tab_map = build_tabs(view_is_admin=True)
        render_tab_contents(tabs, tab_map, view_is_admin=True)

    with mode_student:
        tabs, tab_map = build_tabs(view_is_admin=False)
        render_tab_contents(tabs, tab_map, view_is_admin=False)

else:
    (mode_student,) = st.tabs(["🧒 학생"])
    with mode_student:
        tabs, tab_map = build_tabs(view_is_admin=False)
        render_tab_contents(tabs, tab_map, view_is_admin=False)
