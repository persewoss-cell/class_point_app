...
if "🏦 내 통장" in tabs:
    with tab_map["🏦 내 통장"]:
        ...
        if trade_admin_ok:
            ...
            # 3) 신용도: 저장값/계산값을 안전하게 가져오기 (0으로만 뜨는 문제 방지)
            credit_score, credit_grade = 0, 10
            try:
                # (1) 공용 안전 함수가 있으면 우선
                if callable(globals().get("_safe_credit")):
                    credit_score, credit_grade = _safe_credit(str(student_id))
                else:
                    credit_score, credit_grade = _calc_credit_score_for_student(str(student_id))

                # (2) 계산/조회 결과가 비정상(등급 0 등)일 때 students 저장값으로 보완
                if int(credit_grade or 0) <= 0:
                    snap = db.collection("students").document(str(student_id)).get()
                    if snap.exists:
                        d = snap.to_dict() or {}
                        credit_score = int(d.get("credit_score", credit_score) or credit_score)
                        credit_grade = int(d.get("credit_grade", credit_grade) or credit_grade)

                # (3) 그래도 등급이 0이거나 범위를 벗어나면 점수로 재산정
                g = int(credit_grade or 0)
                if g < 1 or g > 10:
                    credit_grade = _score_to_grade(int(credit_score or 0))
            except Exception:
                credit_score, credit_grade = 0, 10

            # 4) 투자 현재평가(종목별) + 합계  / 투자 원금(종목별) + 합계
            #    - 총자산은 '현재평가금' 기준으로 반영
            invest_text, invest_value_total = "없음", 0
            invest_principal_text, invest_principal_total = "없음", 0
            try:
            ...
            try:
                invest_principal_text, invest_principal_total = _get_invest_principal_by_student_id(str(student_id))
            except Exception:
                invest_principal_text, invest_principal_total = "없음", 0
            ...
            # ✅ (PATCH) 요약 표시: '총자산' 줄 제거 + 글자/줄간격 축소
            st.markdown(
                """
            <style>
            .bank-summary{
            ...
            .bank-summary .text strong{font-weight:800;}
            </style>
            """,
                unsafe_allow_html=True
            )
            ...
            <div class="bank-summary">
              <div class="row"><div class="emoji">💰</div><div class="text"><strong>통장 잔액:</strong> {int(balance)}드림</div></div>
              <div class="row"><div class="emoji">🏦</div><div class="text"><strong>적금 금액:</strong> {int(total_savings_principal)}드림</div></div>
              <div class="row"><div class="emoji">🪙</div><div class="text"><strong>투자 원금:</strong> 총 {int(invest_principal_total)}드림({invest_principal_text})</div></div>
              <div class="row"><div class="emoji">📈</div><div class="text"><strong>현재 평가:</strong> 총 {int(invest_value_total)}드림({invest_text})</div></div>
              <div class="row"><div class="emoji">💼</div><div class="text"><strong>직업:</strong> {job_name}</div></div>
              <div class="row"><div class="emoji">💳</div><div class="text"><strong>신용도:</strong> {int(credit_grade)}등급({int(credit_score)}점)</div></div>
            </div>
            """,
                unsafe_allow_html=True
            )
