...
if is_admin:
    ...
    if inv_ok:
        ...
        tab_map["🛠️ 관리자"] = tab_objs[4]
        extra_start = 5
    else:
        tab_map["🎯 목표"] = tab_objs[2]
        tab_map["🛠️ 관리자"] = tab_objs[3]
        extra_start = 4
...
if "🛠️ 관리자" in tabs:
    with tab_map["🛠️ 관리자"]:
        ...
        admin_candidates = ["🔎 개별조회","💼 직업/월급","🏛️ 국세청(국고)","📊 통계청","💳 신용등급"]
        allowed = []
        if is_admin:
            allowed = admin_candidates
        else:
            for t in admin_candidates:
                # tab::<탭이름> 권한이면 허용
                if has_tab_access(my_perms, t, is_admin=False):
                    allowed.append(t)
                    continue
                # 역할/permission 기반도 인정(기존과 동일)
                if t == "🏛️ 국세청(국고)" and (can(my_perms,"treasury_read") or can(my_perms,"treasury_write")):
                    allowed.append(t)
                elif t == "📊 통계청" and can(my_perms,"stats_write"):
                    allowed.append(t)
                elif t == "💳 신용등급" and can(my_perms,"credit_write"):
                    allowed.append(t)
                elif t == "💼 직업/월급" and can(my_perms,"jobs_write"):
                    allowed.append(t)

        if not allowed:
            st.info("권한이 없습니다.")
        else:
            disp = {"🔎 개별조회":"개별조회","💼 직업/월급":"직업","🏛️ 국세청(국고)":"국세청","📊 통계청":"통계청","💳 신용등급":"신용"}
            subs = st.tabs([disp.get(t,t) for t in allowed])
            for t, sub in zip(allowed, subs):
                with sub:
                    if t == "🔎 개별조회":
                        render_tab_ind_view()
                    elif t == "💼 직업/월급":
...
if "👥 계정 정보/활성화" in tabs:
    with tab_map["👥 계정 정보/활성화"]:
        ...
        # ✅ 부여 가능한 탭(관리자 탭 전부, 단 계정 정보/활성화 제외)
